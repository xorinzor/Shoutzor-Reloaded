...
class tracks {
	...
	public function search($title = '', $page = 1, $limit = 20) {
		$title = cl::g("mysql")->mres(urldecode($title));
		...
		$query 	= 	"SELECT 
					...
					OR 
						filename LIKE '%$title%'
					OR 
						id IN (SELECT track FROM `track_artist` WHERE artist IN (SELECT id FROM `artist` WHERE name LIKE '%$title%')) 
					LIMIT ".($page * $limit).", $limit";

		$query = cl::g("mysql")->query($query) or debug::addLine("error", "could not execute search query, error: ".cl::g("mysql")->getError(), __FILE__, __LINE__);
	...
	public function addTrack(track $track) {
		...
		$query = cl::g("mysql")->query("SELECT id FROM `track` WHERE crc='{$track->getFile()->getCRC()}' OR filename='".addslashes($track->getFile()->getFileName())."'") or debug::addLine("error", "Could not select track from the database, error: ".cl::g("mysql")->getError(), __FILE__, __LINE__);
		if($query->num_rows > 0):
